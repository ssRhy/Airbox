...
import logging
import concurrent.futures  # 用于多线程解码add
...
class YOLOv8:
    ...
    def __call__(self, img_list):
        ...
        ori_size_list = [(img.shape[1], img.shape[0]) for img in img_list]  # 批量获取尺寸add
        start_time = time.time()

        # 批量预处理
        preprocessed_data = [self.preprocess(img) for img in img_list]
        preprocessed_img_list, ratio_list, txy_list = zip(*preprocessed_data)
...
def main(args):
    ...
    def decode_image(img_file):
        ...
        return src_img, time.time() - start_time
    ...
    if os.path.isdir(args.input): 
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            for root, dirs, filenames in os.walk(args.input):
                filenames.sort()
                img_files = [os.path.join(root, filename) for filename in filenames
                             if os.path.splitext(filename)[-1].lower() in ['.jpg', '.png', '.jpeg', '.bmp', '.webp']]
                for img_file, result in zip(img_files, executor.map(decode_image, img_files)):
                    if result is None:
                        continue
                    src_img, decode_duration = result
                    decode_time += decode_duration
                    img_list.append(src_img)
                    filename_list.append(os.path.basename(img_file))
                    cn += 1
